...
public final class AssetTransfer implements ContractInterface {
    ...
    /**
     * Duplicates an existing asset and assigns the duplicated asset to new owner
     *
     * @param ctx             the transaction context
     * @param originalAssetID the ID of the asset being duplicated
     * @param newOwner        new owner
     * @return the duplicate asset
     * @throws ChaincodeException (see {@link #ReadAsset(Context, String)}, {@link #CreateAsset(Context, String, String, int, String, int)})
     */
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public Asset DuplicateAsset(final Context ctx, final String originalAssetID, final String newOwner) {

        // first retrieve the said asset -> this method handles if asset is missing or not.
        // So I don't have to check manually -> by calling AssetExists() method myself
        Asset original = ReadAsset(ctx, originalAssetID);

        // create a duplicate of `original asset` with `new owner`
        // this will automatically add id to context (stub)
        // because I call the built-in methods, I don't have to mess around with context myself
        @SuppressWarnings("UnnecessaryLocalVariable")
        Asset duplicate = CreateAsset(
                ctx,
                original.getAssetID(),
                original.getColor(),
                original.getSize(),
                // assign new owner
                newOwner,
                original.getAppraisedValue()
        );

        // finally, return the duplicated asset
        return duplicate;
    }
}
